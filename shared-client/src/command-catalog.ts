import type { CommandType } from "./types.js";

export type PublicCommandId = CommandType;

export type PublicCommandCategory =
  | "observe"
  | "build"
  | "research"
  | "management"
  | "dyson";

export type CommandPermissionCategory =
  | "observe"
  | "build"
  | "combat"
  | "research"
  | "management";

export interface PublicCommandDefinition {
  id: PublicCommandId;
  apiCommandName: CommandType;
  cliCommandName?: string;
  category: PublicCommandCategory;
  permissionCategory: CommandPermissionCategory;
  layer: "galaxy" | "system" | "planet";
  requiresActivePlanet: boolean;
  webSurface: "required" | "optional" | "hidden";
}

export const PUBLIC_COMMAND_DEFINITIONS: PublicCommandDefinition[] = [
  {
    id: "install_logistics_vehicle",
    apiCommandName: "install_logistics_vehicle",
    cliCommandName: "install_logistics_vehicle",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "configure_traffic_monitor",
    apiCommandName: "configure_traffic_monitor",
    cliCommandName: "configure_traffic_monitor",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "configure_splitter",
    apiCommandName: "configure_splitter",
    cliCommandName: "configure_splitter",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "scan_galaxy",
    apiCommandName: "scan_galaxy",
    cliCommandName: "scan_galaxy",
    category: "observe",
    permissionCategory: "observe",
    layer: "galaxy",
    requiresActivePlanet: false,
    webSurface: "required",
  },
  {
    id: "scan_system",
    apiCommandName: "scan_system",
    cliCommandName: "scan_system",
    category: "observe",
    permissionCategory: "observe",
    layer: "system",
    requiresActivePlanet: false,
    webSurface: "required",
  },
  {
    id: "scan_planet",
    apiCommandName: "scan_planet",
    cliCommandName: "scan_planet",
    category: "observe",
    permissionCategory: "observe",
    layer: "planet",
    requiresActivePlanet: false,
    webSurface: "required",
  },
  {
    id: "build",
    apiCommandName: "build",
    cliCommandName: "build",
    category: "build",
    permissionCategory: "build",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "move",
    apiCommandName: "move",
    cliCommandName: "move",
    category: "build",
    permissionCategory: "combat",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "attack",
    apiCommandName: "attack",
    cliCommandName: "attack",
    category: "management",
    permissionCategory: "combat",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "produce",
    apiCommandName: "produce",
    cliCommandName: "produce",
    category: "build",
    permissionCategory: "build",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "upgrade",
    apiCommandName: "upgrade",
    cliCommandName: "upgrade",
    category: "build",
    permissionCategory: "build",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "demolish",
    apiCommandName: "demolish",
    cliCommandName: "demolish",
    category: "build",
    permissionCategory: "build",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "configure_logistics_station",
    apiCommandName: "configure_logistics_station",
    cliCommandName: "configure_logistics_station",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "configure_distributor",
    apiCommandName: "configure_distributor",
    cliCommandName: "configure_distributor",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "install_logistics_bot",
    apiCommandName: "install_logistics_bot",
    cliCommandName: "install_logistics_bot",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "uninstall_logistics_bot",
    apiCommandName: "uninstall_logistics_bot",
    cliCommandName: "uninstall_logistics_bot",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "configure_mecha_logistics",
    apiCommandName: "configure_mecha_logistics",
    cliCommandName: "configure_mecha_logistics",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "configure_logistics_slot",
    apiCommandName: "configure_logistics_slot",
    cliCommandName: "configure_logistics_slot",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "cancel_construction",
    apiCommandName: "cancel_construction",
    cliCommandName: "cancel_construction",
    category: "build",
    permissionCategory: "build",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "restore_construction",
    apiCommandName: "restore_construction",
    cliCommandName: "restore_construction",
    category: "build",
    permissionCategory: "build",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "start_research",
    apiCommandName: "start_research",
    cliCommandName: "start_research",
    category: "research",
    permissionCategory: "research",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "cancel_research",
    apiCommandName: "cancel_research",
    cliCommandName: "cancel_research",
    category: "research",
    permissionCategory: "research",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "set_recipe",
    apiCommandName: "set_recipe",
    cliCommandName: "set_recipe",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "transfer_item",
    apiCommandName: "transfer_item",
    cliCommandName: "transfer",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "refuel_mecha",
    apiCommandName: "refuel_mecha",
    cliCommandName: "refuel_mecha",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "mine_resource",
    apiCommandName: "mine_resource",
    cliCommandName: "mine_resource",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "craft_item",
    apiCommandName: "craft_item",
    cliCommandName: "craft_item",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "cancel_mecha_job",
    apiCommandName: "cancel_mecha_job",
    cliCommandName: "cancel_mecha_job",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "switch_active_planet",
    apiCommandName: "switch_active_planet",
    cliCommandName: "switch_active_planet",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: false,
    webSurface: "required",
  },
  {
    id: "set_ray_receiver_mode",
    apiCommandName: "set_ray_receiver_mode",
    cliCommandName: "set_ray_receiver_mode",
    category: "dyson",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "set_energy_exchanger_mode",
    apiCommandName: "set_energy_exchanger_mode",
    cliCommandName: "set_energy_exchanger_mode",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "deploy_squad",
    apiCommandName: "deploy_squad",
    cliCommandName: "deploy_squad",
    category: "management",
    permissionCategory: "combat",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "commission_fleet",
    apiCommandName: "commission_fleet",
    cliCommandName: "commission_fleet",
    category: "management",
    permissionCategory: "combat",
    layer: "system",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "fleet_assign",
    apiCommandName: "fleet_assign",
    cliCommandName: "fleet_assign",
    category: "management",
    permissionCategory: "combat",
    layer: "system",
    requiresActivePlanet: false,
    webSurface: "optional",
  },
  {
    id: "fleet_attack",
    apiCommandName: "fleet_attack",
    cliCommandName: "fleet_attack",
    category: "management",
    permissionCategory: "combat",
    layer: "system",
    requiresActivePlanet: false,
    webSurface: "optional",
  },
  {
    id: "fleet_disband",
    apiCommandName: "fleet_disband",
    cliCommandName: "fleet_disband",
    category: "management",
    permissionCategory: "combat",
    layer: "system",
    requiresActivePlanet: false,
    webSurface: "optional",
  },
  {
    id: "fleet_move",
    apiCommandName: "fleet_move",
    cliCommandName: "fleet_move",
    category: "management",
    permissionCategory: "combat",
    layer: "system",
    requiresActivePlanet: false,
    webSurface: "optional",
  },
  {
    id: "task_force_create",
    apiCommandName: "task_force_create",
    cliCommandName: "task_force_create",
    category: "management",
    permissionCategory: "combat",
    layer: "planet",
    requiresActivePlanet: false,
    webSurface: "optional",
  },
  {
    id: "task_force_assign",
    apiCommandName: "task_force_assign",
    cliCommandName: "task_force_assign",
    category: "management",
    permissionCategory: "combat",
    layer: "system",
    requiresActivePlanet: false,
    webSurface: "optional",
  },
  {
    id: "task_force_set_stance",
    apiCommandName: "task_force_set_stance",
    cliCommandName: "task_force_set_stance",
    category: "management",
    permissionCategory: "combat",
    layer: "planet",
    requiresActivePlanet: false,
    webSurface: "optional",
  },
  {
    id: "task_force_deploy",
    apiCommandName: "task_force_deploy",
    cliCommandName: "task_force_deploy",
    category: "management",
    permissionCategory: "combat",
    layer: "system",
    requiresActivePlanet: false,
    webSurface: "optional",
  },
  {
    id: "theater_create",
    apiCommandName: "theater_create",
    cliCommandName: "theater_create",
    category: "management",
    permissionCategory: "combat",
    layer: "planet",
    requiresActivePlanet: false,
    webSurface: "optional",
  },
  {
    id: "theater_define_zone",
    apiCommandName: "theater_define_zone",
    cliCommandName: "theater_define_zone",
    category: "management",
    permissionCategory: "combat",
    layer: "system",
    requiresActivePlanet: false,
    webSurface: "optional",
  },
  {
    id: "theater_set_objective",
    apiCommandName: "theater_set_objective",
    cliCommandName: "theater_set_objective",
    category: "management",
    permissionCategory: "combat",
    layer: "system",
    requiresActivePlanet: false,
    webSurface: "optional",
  },
  {
    id: "blockade_planet",
    apiCommandName: "blockade_planet",
    cliCommandName: "blockade_planet",
    category: "management",
    permissionCategory: "combat",
    layer: "planet",
    requiresActivePlanet: false,
    webSurface: "optional",
  },
  {
    id: "landing_start",
    apiCommandName: "landing_start",
    cliCommandName: "landing_start",
    category: "management",
    permissionCategory: "combat",
    layer: "planet",
    requiresActivePlanet: false,
    webSurface: "optional",
  },
  {
    id: "blueprint_create",
    apiCommandName: "blueprint_create",
    cliCommandName: "blueprint_create",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "blueprint_set_component",
    apiCommandName: "blueprint_set_component",
    cliCommandName: "blueprint_set_component",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "blueprint_validate",
    apiCommandName: "blueprint_validate",
    cliCommandName: "blueprint_validate",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "blueprint_finalize",
    apiCommandName: "blueprint_finalize",
    cliCommandName: "blueprint_finalize",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "blueprint_variant",
    apiCommandName: "blueprint_variant",
    cliCommandName: "blueprint_variant",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "queue_military_production",
    apiCommandName: "queue_military_production",
    cliCommandName: "queue_military_production",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "refit_unit",
    apiCommandName: "refit_unit",
    cliCommandName: "refit_unit",
    category: "management",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
  {
    id: "launch_solar_sail",
    apiCommandName: "launch_solar_sail",
    cliCommandName: "launch_solar_sail",
    category: "dyson",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "launch_rocket",
    apiCommandName: "launch_rocket",
    cliCommandName: "launch_rocket",
    category: "dyson",
    permissionCategory: "management",
    layer: "planet",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "build_dyson_node",
    apiCommandName: "build_dyson_node",
    cliCommandName: "build_dyson_node",
    category: "dyson",
    permissionCategory: "build",
    layer: "system",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "build_dyson_frame",
    apiCommandName: "build_dyson_frame",
    cliCommandName: "build_dyson_frame",
    category: "dyson",
    permissionCategory: "build",
    layer: "system",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "build_dyson_shell",
    apiCommandName: "build_dyson_shell",
    cliCommandName: "build_dyson_shell",
    category: "dyson",
    permissionCategory: "build",
    layer: "system",
    requiresActivePlanet: true,
    webSurface: "required",
  },
  {
    id: "demolish_dyson",
    apiCommandName: "demolish_dyson",
    cliCommandName: "demolish_dyson",
    category: "dyson",
    permissionCategory: "build",
    layer: "system",
    requiresActivePlanet: true,
    webSurface: "optional",
  },
];

export const PUBLIC_COMMAND_CATALOG = Object.fromEntries(
  PUBLIC_COMMAND_DEFINITIONS.map((definition) => [definition.id, definition]),
) as Record<PublicCommandId, PublicCommandDefinition>;

export const REQUIRED_WEB_PUBLIC_COMMAND_IDS = PUBLIC_COMMAND_DEFINITIONS.filter(
  (definition) => definition.webSurface === "required",
).map((definition) => definition.id) as PublicCommandId[];

export function getPublicCommandDefinition(id: string) {
  return PUBLIC_COMMAND_CATALOG[id as PublicCommandId] ?? null;
}

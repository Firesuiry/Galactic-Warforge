...
func TestPlanetSceneEndpointTranslatesViewportQuery(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/world/planets/planet-1-1/scene?x=2&y=3&width=4&height=2", nil)
	req.Header.Set("Authorization", "Bearer key1")
	rec := httptest.NewRecorder()
	srv.Handler().ServeHTTP(rec, req)
	...
	if bounds["x"] != float64(2) || bounds["y"] != float64(3) || bounds["width"] != float64(4) || bounds["height"] != float64(2) {

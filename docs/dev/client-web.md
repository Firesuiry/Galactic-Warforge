...
- 星图直操作（期7b）：银河层点舰队徽标直选舰队（同系多艘循环点选 `pickFleetInSystem`），徽标加金色高亮环（frozen 门控脉冲），底部浮出舰队情境条 `FleetSelectionBar`（舰队 id/状态/阵型/装甲/结构/所在星系 + 动作按钮，对齐 PlanetSelectionBar 范式）；动作只接 server 真实指令——`fleet_assign`（编队）、`fleet_disband`（解散）、`fleet_attack`（攻击模式：自动聚焦舰队所在星系，点行星经 `resolveFleetAttackTarget` 取该行星首个可锁定敌方接触组装指令，无可锁定目标本地拦截）；server 无舰队移动/跃迁指令且 fleet_attack 限同星系，故不做"点星系机动"。指令提交复用 `useWarCommand` 管道（回执 banner + 音效 + war-fleets/system-runtime 失效）；选中态存 starmap store（`selectedFleetId` + `interactionMode`），Esc/右键/点空地逐层退出，星系点选/双击导航行为不变

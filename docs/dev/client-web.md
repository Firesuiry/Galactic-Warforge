...
- 行星工作台新增「生产」「发展」标签，左上提供快捷入口；3D 和平面战术共用产线诊断、配方批次用料规划、携带配方的地图建造和六阶段发展路线。
- 设施诊断与发展目标仅核对当前载入区域，科技和运行态采用服务端数据；不将配方估算值作为实时产率或给出虚构完成奖励。
- 3D 行星支持流畅、均衡、精细三档；离屏 MSAA、阴影和辉光可即时切换，偏好保存在 localStorage。岸线依据已探索的球面邻格生成。
- 具体边界、操作和验证命令见 [工业发展与画质增强](../guide/工业发展与画质增强.md)。

## 当前行星网格协议

行星使用 `cube_sphere` 六面球面网格，`map_width=3N`、`map_height=2N` 是存储图集尺寸，N 来自 `surface.face_size`。全球和近景使用等角立方体球面投影，禁止恢复经纬条带或极区模型缩放。平面战术视图展示的是六面图集，图集边缘相邻不能直接解释为地表相邻。

scene 的跨面 `surface_patches` 必须按独立 bounds 合成；overview 必须使用响应中的实际 step，它整除 N。路径由 `GET /world/planets/{id}/path` 在已探索区域规划，`stop_range` 指允许距目标停下的球面步数，范围 0..128；建造靠近使用执行体操作范围。返回 waypoints 按单位 move_range 分段，空 waypoints 且 reachable=true 表示已在范围内；reachable=false 时不能直接尝试建造。路径查询最多 512 步，局势变化后的移动仍可能被后端拒绝，应刷新并重规划。

建筑布局蓝图的选区和旋转粘贴边界须在单面内，跨面布局需拆分。战争蓝图的科技、组件和定型流程不受这一地表选区规则影响。

完整面序、坐标协议和当前验证记录见 [立方体球面网格](../guide/立方体球面网格.md) 与 [3D 表现开发指南](../guide/3D表现开发指南.md)。旧的 2026-09-10 经纬投影测试仅是历史记录，不代表本次六面网格验收。
...
- 星图直操作（期7b）：银河层点舰队徽标直选舰队（同系多艘循环点选 `pickFleetInSystem`），徽标加金色高亮环（frozen 门控脉冲），底部浮出舰队情境条 `FleetSelectionBar`（舰队 id/状态/阵型/装甲/结构/所在星系 + 动作按钮，对齐 PlanetSelectionBar 范式）；动作只接 server 真实指令——`fleet_assign`（编队）、`fleet_disband`（解散）、`fleet_attack`（攻击模式：自动聚焦舰队所在星系，点行星经 `resolveFleetAttackTarget` 取该行星首个可锁定敌方接触组装指令，无可锁定目标本地拦截）；指令提交复用 `useWarCommand` 管道（回执 banner + 音效 + war-fleets/system-runtime 失效）；选中态存 starmap store（`selectedFleetId` + `interactionMode`），Esc/右键/点空地逐层退出，星系点选/双击导航行为不变
- 舰队跨星系跃迁（期7c）：`FleetSelectionBar` 新增「跃迁」动作（仅 `state=idle` 且非跃迁中的舰队可用）——进入 move 模式并退回银河层，点目标星系经 `resolveFleetMoveTarget` 本地校验（目标是当前星系/无航线连接/交战中/已跃迁中均本地拦截，航线图与场景渲染同一套 `computeSystemLanes` k 近邻结果）后下达 `fleet_move`；跃迁中舰队不进星系徽标（`summarizeFleetsBySystem`/`pickFleetInSystem` 跳过），改由 `transitFleetRenders` 按 `1-remaining/total` 在起止星系间直线插值，scene 新增跃迁光点层（青金色 glow 光点 + 向出发端回退的短拖尾，位置纯数据驱动，frozen 下静止于当前进度），到达后回到目标星系徽标；情境条对跃迁中舰队显示「跃迁 A→B · 剩余 n/m tick」并禁用全部动作（与 server 阻断一致）；星图路由下 `war-fleets` 查询带 2s 轮询让跃迁进度持续推进；通知新增 `fleet_move_started`（info「🚀 舰队跃迁：A→B」+ uiClick）/ `fleet_arrived`（success「✓ 舰队抵达：B」+ commandOk），href 均指向 `/galaxy`；星图路由此前没有 SSE 源，新增 `use-starmap-notify.ts` 通知桥（game 事件 → 全局 toast，挂在 StarmapView）保证跃迁事件在星图上也能弹出；`fleet_move_started`/`fleet_arrived` 已加入 `shouldRefreshWarFleets`/`shouldRefreshWarSystemRuntime` 失效表
...
- 窄屏/移动端会保留地图首屏，并把右侧区域收口成 `工作台 / 选中对象 / 生产 / 发展 / 活动流` 五个页签，默认进入 `工作台`
...
- 行星视觉三连（期10，试玩视频评审）：①默认/回家/概览双击落点缩放档统一提到 48px/tile（`DEFAULT_PLANET_ZOOM_INDEX`/`PLANET_HOME_ZOOM_INDEX` 6/8→9，`DEFAULT_PLANET_SCENE_TILE_SIZE` 兜底同步 48），解决 32px 档建筑/地块图标偏小、缺乏沉浸感；②左上信息片与底部建造栏紧凑化（字号/内距收窄，建造卡图标 30→26、卡片 min-width 88→74），右侧工作台抽屉维持可折叠（收起为右缘把手、面板状态保留），地图可视区域显著变大；③物流可视化（`planet-logistics-flow.ts` 纯逻辑 + `planet-scene.ts` 视图绑定）——传送带 `conveyor.buffer`（scene 快照本已直出，shared-client 补 `BuildingConveyorState.buffer` 类型声明）有货时挂货物圆点（颜色 = 资源调色板，≤4 粒抽样），ticker 驱动沿 `conveyor.output` 匀速循环流动（流速按 throughput 近似，钳 1.45~3 tile/s）；running 采集建筑挂"矿石被采出"上抛脉冲（颜色 = `collect.resource_kind`，2.6s 周期 + id hash 错相）；≥8px/tile 档显示，frozen 停在确定相位供截图

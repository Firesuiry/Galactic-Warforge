...
describe("shared api client", () => {
  it("在未显式传入 event_types 时为事件快照补默认类型", async () => {
    ...
    await expect(
      client.fetchEventSnapshot({ limit: 8 }),
  ...
  it("会为行星场景接口序列化视口参数", async () => {
    ...
    await expect(
      client.fetchPlanetScene("planet-1-1", {
        x: 96,
        y: 64,
        width: 160,
        height: 128,
  ...
  it("会为行星总览接口序列化下采样参数", async () => {
    ...
    await expect(
      client.fetchPlanetOverview("planet-1-1", {
        step: 100,
      }),
    ).resolves.toMatchObject({
      step: 100,
  ...
  it("会为行星检视接口序列化实体定位参数", async () => {
    ...
    await expect(
      client.fetchPlanetInspect("planet-1-1", {
        entityKind: "building",
  ...
  it("会序列化 configure_logistics_station 命令", async () => {
    const fetchMock = vi.fn(
      (input: string | URL | Request, init?: RequestInit) => {
        const url = new URL(String(input));

        expect(url.pathname).toBe("/commands");
        expect(init?.method).toBe("POST");
        expect(JSON.parse(String(init?.body))).toMatchObject({
          commands: [
            {
              type: "configure_logistics_station",
              target: { layer: "planet", entity_id: "pls-1" },
              payload: {
                drone_capacity: 12,
                input_priority: 2,
                output_priority: 4,
        ...
        return Promise.resolve(
          jsonResponse({
            request_id: "req-logistics-station",
            accepted: true,
            enqueue_tick: 88,
            results: [
              {
    ...
    await expect(
      client.cmdConfigureLogisticsStation("pls-1", {
        droneCapacity: 12,
        inputPriority: 2,
        outputPriority: 4,
        interstellar: {
  ...
  it("会序列化 configure_logistics_slot 命令", async () => {
    const fetchMock = vi.fn(
      (input: string | URL | Request, init?: RequestInit) => {
        const url = new URL(String(input));
          ...
          commands: [
            {
              ...
              target: { layer: "planet", entity_id: "ils-1" },
              payload: {
                scope: "interstellar",
                item_id: "hydrogen",
                mode: "demand",
    ...
    await expect(
      client.cmdConfigureLogisticsSlot("ils-1", {
        scope: "interstellar",
        itemId: "hydrogen",
        mode: "demand",
        localStorage: 80,

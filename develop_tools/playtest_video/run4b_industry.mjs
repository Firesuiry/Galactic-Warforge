...
async function main() {
  browser = await chromium.launch({ headless: true });
  await startSegment();
  logEvent('milestone', '产线收官开始');

  // S1: 拆闲置研究站回款（研究已全部完成）
  await demolishAt({ x: 3, y: 2 }, 'matrix_lab');
  const m1 = await minerals();
  logEvent('info', `回款后矿产: ${m1}`);

  // S2: 重建供电（两台风机贴基地/电网节点）
  await buildAt('wind_turbine', { x: 2, y: 3 });
  await buildAt('wind_turbine', { x: 3, y: 4 });
  // S3: 储物仓贴分拣器 (4,2)
  await buildAt('depot_mk1', { x: 4, y: 2 });

  // S4: 验证矿机解封（库存下降 + 矿产回升）
  const oreBefore = await (async () => {
    const s = await getScene();
    const miner = Object.values(s.buildings ?? {}).find((b) => b.type === 'mining_machine');
    return miner?.storage?.inventory?.silicon_ore ?? 0;
  })();
  logEvent('info', `矿机硅库存基线: ${oreBefore}`);
  await waitMinerals(40, 300, '产线打通，经济复苏');
  ...
  await buildAt('gauss_turret', { x: 1, y: 3 });
  await waitMinerals(90, 360, '攒第二座炮塔');
  await buildAt('gauss_turret', { x: 4, y: 0 });

...
const OUT = '.run/video-playtest/session3';
...
async function main() {
  browser = await chromium.launch({ headless: true });
  await startSegment();
  logEvent('milestone', '救赎行动开始：回收前哨，重建主矿');

  // S1: 开赴前哨，拆除闲置特斯拉塔与风机回款
  await moveExecutorTo({ x: 6, y: 14 }, 2);
  await demolishAt({ x: 7, y: 15 }, 'tesla_tower');
  await demolishAt({ x: 6, y: 15 }, 'wind_turbine');
  const m1 = await minerals();
  logEvent('info', `回收后矿产: ${m1}`);

  // S2: 回师基地，重建风机恢复供电
  await moveExecutorTo({ x: 3, y: 3 }, 2);
  await buildAt('wind_turbine', { x: 3, y: 4 });
  {
    // 若 (3,4) 被占，换 (3,2)
    const s = await getScene();
    if (!Object.values(s.buildings ?? {}).some((b) => b.type === 'wind_turbine' && b.position.x === 3 && b.position.y === 4)) {
      await buildAt('wind_turbine', { x: 3, y: 2 });
    }
  }
  // 等矿机恢复 running
  logEvent('wait', '等待矿机复电');
  {
    const start = elapsed();
    while (elapsed() - start < 120) {
      const s = await getScene();
      const miner = Object.values(s.buildings ?? {}).find((b) => b.type === 'mining_machine');
      if (miner && (miner.runtime?.state === 'running')) {
        logEvent('milestone', '矿机复电复产 ✓');
        break;
      }
      await wander(10, '等待矿机复电');
    }
  }

  // S3: 等经济冲起来（矿物流向储物仓，矿产回升）
  await waitMinerals(60, 300, '产线打通，经济复苏');
  ...
  // S5: 炮塔×2
  await waitMinerals(90, 480, '攒矿建炮塔');
  await buildAt('gauss_turret', { x: 2, y: 4 });
  await waitMinerals(90, 360, '攒第二座炮塔');
  await buildAt('gauss_turret', { x: 5, y: 4 });

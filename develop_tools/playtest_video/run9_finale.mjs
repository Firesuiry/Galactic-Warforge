...
const OUT = '.run/video-playtest/session8';
...
async function main() {
  browser = await chromium.launch({ headless: true });
  await startSegment();
  logEvent('milestone', '收官行动开始：二次创业');

  // S1: 拆除已展示的熔炉回款，转型军工与二矿
  {
    const s = await getScene();
    const smelter = Object.values(s.buildings ?? {}).find((b) => b.type === 'arc_smelter');
    if (smelter) {
      await demolishAt({ x: smelter.position.x, y: smelter.position.y }, 'arc_smelter');
    }
  }
  const m0 = await minerals();
  logEvent('info', `回款后矿产: ${m0}`);

  // S2: 第二台矿机压第二硅矿点 (6,2)，恢复收入
  await buildAt('mining_machine', { x: 6, y: 2 });
  {
    const start = elapsed();
    while (elapsed() - start < 120) {
      const s = await getScene();
      const miners = Object.values(s.buildings ?? {}).filter((b) => b.type === 'mining_machine' && b.runtime?.state === 'running');
      if (miners.length >= 1) { logEvent('milestone', '第二矿机投产 ✓'); break; }
      await wander(10, '等待二矿投产');
    }
  }

  // S3: 炮塔
  await waitMinerals(85, 420, '攒矿建炮塔');

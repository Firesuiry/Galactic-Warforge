...
const OUT = '.run/video-playtest/session7';
...
async function main() {
  browser = await chromium.launch({ headless: true });
  await startSegment();
  logEvent('milestone', '终局行动开始：产线已通，全线投产');

  // S1: 展示已贯通的产线（矿机→皮带→仓储），验证矿产回升
  ...
  logEvent('info', `矿产基线: ${m0}`);
  {
    ...
    while (elapsed() - start < 120) {
      ...
      if (m > m0 + 20) { logEvent('milestone', '矿机解封，矿产回升 ✓'); await shot('economy-flowing'); break; }
      const s = await getScene();
      ...
      logEvent('info', `矿机库存 ${miner?.storage?.inventory?.silicon_ore ?? 0} / 仓储 ${depot?.storage?.inventory?.silicon_ore ?? 0} / 矿产 ${m}`);
      await wander(12, '观察矿流');
    }
  }
